...
- Если в prompt_ru есть несколько равноценных вариантов, разделяй их запятой, никогда не через «/»: «седоватый, с проседью», а не «седоватый / с проседью».
...
- forms_basic показывается вместе с ответом всегда, forms_extended — по нажатию. Каждая строка: label — короткая польская помета, value — форма, несколько форм через « · ». Только по-польски. Для fraza и inne оба списка пустые. Если формы не существует, строку пропускай, не выдумывай.
  ...
  - czasownik: forms_basic — «aspekt» (вид и видовая пара, например «ndk. → dk. zrobić»), затем «ja · ty · oni» настоящего времени, затем «tryb rozk.» для ty. У глагола совершенного вида настоящего времени нет: вместо него дай czas przyszły prosty (ja · ty · oni) с пометой «cz. przyszły». forms_extended — сначала «cz. przeszły 1 os.» (l.poj., m · ż: «zrobiłem · zrobiłam»), затем «cz. przeszły l.poj.» (m · ż · n), «cz. przeszły l.mn.» (m-os. · nie-m-os.), «imiesłowy» (только те, что существуют для этого вида: у совершенного нет формы на -ący), «forma bezosobowa».
  - przymiotnik: forms_basic — «stopniowanie przymiotnika» (stopień wyższy · najwyższy, например «szybszy · najszybszy»), затем «przysłówek», образованный от него, затем «stopniowanie przysłówka» («szybciej · najszybciej»); forms_extended пустой.
  - przyslowek: forms_basic — «stopniowanie przysłówka» (stopień wyższy · najwyższy, например «szybciej · najszybciej»), затем «przymiotnik», от которого он образован, затем «stopniowanie przymiotnika» («szybszy · najszybszy»); forms_extended пустой.
  - Для обеих строк stopniowanie: если степени образуются описательно, пиши «bardziej zmęczony · najbardziej zmęczony». Если слово не имеет степеней сравнения («martwy», «drewniany»), строку не пропускай, а пиши value «nie stopniuje się». Если парного przysłówek или przymiotnik нет, пропусти и его строку, и его stopniowanie.

...
- **Gemini model** comes from `FISZKI_MODEL`, default **`gemini-3.8-flash`** (user decision, 2026-09-14, after the live model list showed the only GA non-preview Pro model was a generation behind; see spec §5). Named fallback if card quality disappoints: `gemini-2.5-pro`. Structured output is mandatory: every generation request sends `responseMimeType: "application/json"` plus a `responseSchema`, and the response is additionally validated with Zod. Never hand-parse JSON out of a text part, and never strip markdown fences to find it — if a response needs that treatment, the request was built wrong.
...
- **Do not use Turbopack.** `create-next-app` adds `--turbopack` to the `dev` and
  `build` scripts; the plan never asked for it, and in this environment it hangs
  indefinitely compiling a React page (Task 7 lost time to `/logowanie`, and Task 1
  wrongly generalised it to "`next build` hangs"). Measured 2026-09-14 with the flag
  removed: `/logowanie` compiled and served in **2.8 s** (HTTP 200, Polish strings
  present), and `next build` exited **0** with all 8 routes and the middleware built.
  Route handlers always compiled fine either way, which is why Tasks 10 and 11 never
  hit it. Keep both scripts on webpack.
- **Every task ends with `npx tsc --noEmit` clean, before committing.** Vitest
  transpiles without typechecking, so a green suite says nothing about whether
  the project builds. This was not a hypothetical: Task 4 shipped a `TS2345`
  that left the whole project failing to typecheck while its 25/25 suite stayed
  green, and nothing would have caught it until Task 18's `npm run build` —
  fourteen tasks downstream of the cause. A failing typecheck is an **Important**
  review finding, not a cosmetic one. Never silence one with a cast,
  `@ts-expect-error`, or a widened type; those hide exactly the signal the gate
  exists to surface.
...
- Create: `package.json`, `tsconfig.json`, `next.config.ts`, `postcss.config.mjs`, `vitest.config.ts`, `.gitignore`, `.env.example`
  - **No `tailwind.config.ts`.** Tailwind v4 — which is what current tooling
    installs — configures through `@theme` in CSS plus `@tailwindcss/postcss`,
    and has no such file. Creating one would be inert. (Corrected 2026-09-13
    during execution; the original list named it.)
...
Note: **this paragraph is superseded — see the correction below it.** It read:
"`undoLastReview` restores only the scheduler fields, deliberately leaving
`updatedAt` as the review set it — the 'restores the card byte-for-byte' test
passes because `recordReview` and the seed use the same timestamp. If you change
the seed's `updatedAt`, add it to the restore set rather than weakening the test."

**Correction (2026-09-13, found during execution):** the note and this task's own
third test contradict each other. That test does a full-row `toEqual` against a
row captured 60s earlier, so it *does* assert `updatedAt` is restored, and it
fails against the behaviour the note describes. The note anticipated the first
test (which passes only because the seed and `recordReview` share a timestamp)
but not the third.

Neither option the contradiction offers is right. Restoring the *old*
`updatedAt` would assert the row was never touched, which is false — the undo
mutated it and the log retains a row with `undone_at` set. Narrowing the
assertion would discard the coverage proving undo restores every scheduler field
and mutates nothing else.

**Resolution: `undoLastReview` stamps `updatedAt` with the undo timestamp
(`now.getTime()`).** Both of this task's tests then pass verbatim with no
assertion weakened, and the column means what it says — when the row last
changed. Nothing in this plan reads `updatedAt` for logic; it is in neither the
due index nor the queue query.
...
2. Confirm **`gemini-3.8-flash`** appears in the list output and answers a request.
   Put it in `.env.example` as the `FISZKI_MODEL` default and note it in the commit
   message. This model is a decided value, not one to re-derive — but if it is
   absent from the live list, stop and report rather than substituting a neighbour.
...
**Also run the two inputs that actually test the generator's stated job.** The
three above are already canonical, so they exercise none of spec §5's
"responsible for correcting spelling and restoring the dictionary form" — which
is the generator's whole reason for existing, given that mangled diacritics are
"the expected common case". Add:

```bash
#   zlosliwego                   -> expect answer_pl "złośliwy"
#   przyzwyczailem sie do tego   -> expect answer_pl "przyzwyczaiłem się do tego"
```

The first must restore ł/ś **and** lemmatise out of the genitive. The second must
restore ł/ę and **not** lemmatise — it is a phrase, and the rule is that phrases
and sentences stay as dictated. A model that lemmatises the second to an
infinitive is applying the rule mechanically instead of correctly.
...
**Two decisions taken 2026-09-16 that this task implements.**

1. **Delete is soft.** Add `deleted_at INTEGER` to `cards` in a new migration and
   to `lib/db/schema.ts` (Task 1's drift test verifies the two agree).
   `DELETE /api/cards/:id` sets it rather than removing the row, so the `reviews`
   history §7 depends on survives. Every query that selects cards must exclude
   soft-deleted ones — notably `REVIEWABLE` in `lib/review/queue.ts`, which is
   otherwise frozen, and `searchCards`. **The existing `cards_due` partial index
   needs no change**: its `WHERE suspended_at IS NULL AND status = 'ready'` is
   implied by a stricter query, so SQLite can still use it.
2. **A small hand-rolled Markdown renderer** for `pl_forms` answers — bold and
   pipe tables only, which is what the generator emits. No new dependency.

**Also finish spec §4's capture chips here.** Spec §4 line 118 requires each
capture chip to offer "▶ to replay the audio, tap to expand and edit, and swipe
to delete". Task 15 shipped only replay (plus retry, from §11), because the
routes the other two need — `PATCH /api/cards/:id` and `DELETE /api/cards/:id` —
are created by *this* task. Task 15's implementer correctly declined to invent an
API for them and flagged the gap instead.

So once those two routes exist, wire the chip affordances in
`components/CaptureChip.tsx`: tap expands the chip to edit the generated card's
fields, and swipe deletes it. Deleting a chip whose capture has no card yet
should remove the capture; deleting one that has a card should delete the card
and cascade its reviews, which `DELETE /api/cards/:id` already does. Found during
execution 2026-09-15; the plan as written never implemented these.

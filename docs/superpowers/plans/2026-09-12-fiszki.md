...
**Architecture:** One Next.js app (App Router) over one SQLite file that holds everything including image, audio and TTS blobs. Server route handlers own all third-party calls (Whisper, Claude, Google TTS) so no API key reaches the browser. Business logic lives in `lib/` as plain functions over a `db` handle; routes and React components are thin shells around them, which is what makes the test suite possible without a browser.

**Tech Stack:** Next.js 15 (App Router), React 19, TypeScript, Tailwind, better-sqlite3 + Drizzle (typed queries over hand-written SQL migrations), ts-fsrs, Zod, `@anthropic-ai/sdk`, sharp, Vitest + @testing-library/react + fake-indexeddb.
...
- **Dictation is always Polish.** Whisper is called with `language: "pl"` pinned — never auto-detect.
...
- **Claude model** comes from `FISZKI_MODEL`, default `claude-opus-5`. Structured output uses `client.messages.parse()` with `zodOutputFormat`, never assistant prefill and never hand-parsed JSON from a text block.
...
lib/transcribe/index.ts            Transcriber interface + Groq Whisper provider
lib/generate/index.ts              Generator interface + Claude provider + Zod schemas
lib/tts/index.ts                   Synthesizer interface + Google provider + clip cache
...
npm install better-sqlite3 drizzle-orm zod ts-fsrs@4.7.0 @anthropic-ai/sdk sharp idb-keyval
...
Whisper behind a one-function interface, with Polish pinned.

**Files:**
- Create: `lib/transcribe/index.ts`
...
- Produces:
  ...
  - `groqTranscriber(opts: { apiKey: string; model?: string; fetchImpl?: typeof fetch }): Transcriber`
...
import { TranscriptionError, groqTranscriber } from './index'
...
describe('groqTranscriber', () => {
  it('returns the trimmed transcript', async () => {
    const fetchImpl = fakeFetch(Response.json({ text: '  złośliwy  ' }))
    const t = groqTranscriber({ apiKey: 'k', fetchImpl })
    expect(await t.transcribe(audio)).toBe('złośliwy')
  })

  it('pins the language to Polish and never auto-detects', async () => {
    const fetchImpl = fakeFetch(Response.json({ text: 'x' }))
    await groqTranscriber({ apiKey: 'k', fetchImpl }).transcribe(audio)
  ...
  it('sends the API key as a bearer token', async () => {
    const fetchImpl = fakeFetch(Response.json({ text: 'x' }))
    await groqTranscriber({ apiKey: 'secret', fetchImpl }).transcribe(audio)
    const headers = new Headers(fetchImpl.mock.calls[0][1]!.headers)
    expect(headers.get('authorization')).toBe('Bearer secret')
  })

  it('throws a TranscriptionError carrying the status on an API failure', async () => {
    const fetchImpl = fakeFetch(new Response('upstream exploded', { status: 503 }))
    const t = groqTranscriber({ apiKey: 'k', fetchImpl })
    await expect(t.transcribe(audio)).rejects.toThrow(TranscriptionError)
    await expect(t.transcribe(audio)).rejects.toMatchObject({ status: 503 })
  })

  it('throws when the response has no text field', async () => {
    const fetchImpl = fakeFetch(Response.json({ oops: true }))
    await expect(groqTranscriber({ apiKey: 'k', fetchImpl }).transcribe(audio)).rejects.toThrow(
      TranscriptionError,
    )
  })

  it('throws when the transcript is empty, so an empty card is never created', async () => {
    const fetchImpl = fakeFetch(Response.json({ text: '   ' }))
    await expect(groqTranscriber({ apiKey: 'k', fetchImpl }).transcribe(audio)).rejects.toThrow(
      TranscriptionError,
    )
  })
...
export function groqTranscriber(opts: {
  apiKey: string
  model?: string
  ...
  return {
    async transcribe({ bytes, mime }) {
      ...
      form.set('file', new Blob([bytes], { type: mime }), 'capture.webm')
      form.set('model', model)
      // Dictation is always Polish. Auto-detection guesses wrong on single
      // words and silently produces Russian or English spellings.
      ...
      if (!res.ok) {
        throw new TranscriptionError(`transcription failed: ${await res.text()}`, res.status)
      }
...
```bash
git add lib/transcribe
git commit -m "feat: Whisper transcription with Polish pinned"
```
...
Turns a Polish utterance into a Russian→Polish card. Uses structured outputs so the response is schema-valid or an error, never prose to be parsed.
...
- Produces:
  ...
  - `claudeGenerator(opts?: { client?: Pick<Anthropic, 'messages'>; model?: string }): Generator`
  ...
  Every schema field is a **required string**; absence is the empty string, which `toCardFields` maps to `null`. Optional/nullable JSON-schema fields are avoidable complexity here, and "" → null in one tested function is easier to reason about than a partially-populated object.
...
import { GenerationError, claudeGenerator, toCardFields } from './index'
...
describe('claudeGenerator.fromPolish', () => {
  it('returns the parsed card', async () => {
    const { client } = fakeClient(FULL)
    expect(await claudeGenerator({ client }).fromPolish('złośliwy')).toEqual(FULL)
  })

  it('sends the transcript and the configured model', async () => {
    const { client, parse } = fakeClient(FULL)
    await claudeGenerator({ client, model: 'claude-opus-5' }).fromPolish('na wszelki wypadek')
    const params = parse.mock.calls[0][0]
    expect(params.model).toBe('claude-opus-5')
    expect(JSON.stringify(params.messages)).toContain('na wszelki wypadek')
    expect(params.output_config.format).toBeDefined()
  })

  it('instructs the model that answers are Polish and prompts Russian, never English', async () => {
    const { client, parse } = fakeClient(FULL)
    await claudeGenerator({ client }).fromPolish('złośliwy')
  ...
  it('throws GenerationError when the model returns nothing parseable', async () => {
    const { client } = fakeClient(null)
    await expect(claudeGenerator({ client }).fromPolish('x')).rejects.toThrow(GenerationError)
  })

  it('throws GenerationError when the payload fails the schema', async () => {
    const { client } = fakeClient({ answer_pl: 'złośliwy' })
    await expect(claudeGenerator({ client }).fromPolish('x')).rejects.toThrow(GenerationError)
  })

  it('rejects an empty transcript before spending a request', async () => {
    const { client, parse } = fakeClient(FULL)
    await expect(claudeGenerator({ client }).fromPolish('   ')).rejects.toThrow(GenerationError)
    expect(parse).not.toHaveBeenCalled()
  })
})

describe('claudeGenerator.fromImage', () => {
  it('sends the image as a base64 image block', async () => {
    const { client, parse } = fakeClient(FULL)
    await claudeGenerator({ client }).fromImage({ bytes: new Uint8Array([1, 2]), mime: 'image/webp' })
    const content = parse.mock.calls[0][0].messages[0].content as Array<Record<string, unknown>>
    const image = content.find((b) => b.type === 'image') as { source: Record<string, string> }
    expect(image.source.type).toBe('base64')
    expect(image.source.media_type).toBe('image/webp')
    expect(image.source.data).toBe(Buffer.from([1, 2]).toString('base64'))
  })
})

describe('claudeGenerator.forms', () => {
  it('returns a Polish prompt and a Polish answer table', async () => {
    const { client } = fakeClient({ prompt_pl: 'przyzwyczaić się — wszystkie formy', answer_pl: '…' })
    expect(await claudeGenerator({ client }).forms('przyzwyczaić się')).toEqual({
      prompt_pl: 'przyzwyczaić się — wszystkie formy',
      answer_pl: '…',
    })
  })
})
...
export function claudeGenerator(opts: {
  client?: Pick<Anthropic, 'messages'>
  model?: string
} = {}): Generator {
  const client = opts.client ?? new Anthropic()
  const model = opts.model ?? process.env.FISZKI_MODEL ?? 'claude-opus-5'

  async function parse<T>(schema: z.ZodType<T>, system: string, content: unknown): Promise<T> {
    let response
    try {
      ...
      } as Parameters<Pick<Anthropic, 'messages'>['messages']['parse']>[0])
    } catch (err) {
    ...
    if (!parsed.success) throw new GenerationError(`generation returned an unusable payload: ${parsed.error.message}`)
    return parsed.data
  }
  ...
  return {
    async fromPolish(transcript) {
      ...
      return parse(GeneratedCardSchema, SYSTEM, [
        { type: 'text', text: `Продиктовано по-польски: «${text}»\n\nСделай карточку.` },
      ])
    },
    ...
    async fromImage({ bytes, mime }) {
      return parse(GeneratedCardSchema, SYSTEM, [
        {
          type: 'image',
          source: { type: 'base64', media_type: mime, data: Buffer.from(bytes).toString('base64') },
        },
        {
          type: 'text',
    ...
    async forms(lemma) {
      ...
      return parse(GeneratedFormsSchema, FORMS_SYSTEM, [
        { type: 'text', text: `Лемма: «${text}»` },
      ])
    },
  }
}
...
- [ ] **Step 4: Verify against the real API once**

```bash
node --experimental-strip-types -e "
import('./lib/generate/index.ts').then(async (m) => {
  console.log(await m.claudeGenerator().fromPolish('złośliwy'))
  console.log(await m.claudeGenerator().fromPolish('nie ma co liczyć na jego pomoc'))
})"
```

Check: `answer_pl` has correct diacritics, `prompt_ru` is Russian, `prompt_hint` is populated for the single word and empty for the sentence, and no English appears anywhere. If `output_config` is rejected with a 400, the SDK version predates structured outputs — upgrade `@anthropic-ai/sdk` rather than falling back to parsing a text block.

- [ ] **Step 5: Commit**

```bash
git add lib/generate
git commit -m "feat: Claude card generation with structured outputs"
```
...
Ties audio to a card. The failure behaviour here is the whole point: a dictated word must survive a dead Whisper, a dead Claude, and a dead network.
...
describe('processCapture', () => {
  ...
  it('keeps the audio and allows retry when transcription fails', async () => {
    const d = deps({ transcriber: { transcribe: vi.fn().mockRejectedValue(new Error('whisper down')) } })
    const id = createCapture(d.db, AUDIO, NOW)
    await processCapture(d, id, NOW)

    const row = d.db.select().from(captures).where(eq(captures.id, id)).get()!
    expect(row.status).toBe('failed')
    expect(row.error).toContain('whisper down')
    expect(row.audioMediaId).not.toBeNull()
    expect(d.db.select().from(media).all()).toHaveLength(1)
  ...
  it('still creates a card when generation fails, flagged needs_input', async () => {
    const d = deps({ generator: { fromPolish: vi.fn().mockRejectedValue(new Error('claude down')) } })
    const id = createCapture(d.db, AUDIO, NOW)
    await processCapture(d, id, NOW)
    ...
    expect(row.error).toContain('claude down')
  })
 ...
 * Whisper leaves the capture retryable with its audio intact, while a dead
 * Claude still yields a card carrying the word.
 */
...
**Interfaces:**
- Consumes: `Db`, `ttsClips` (Task 1); `putMedia`, `getMedia` (Task 2)
- Produces:
  ...
  - `googleSynthesizer(opts: { apiKey: string; fetchImpl?: typeof fetch }): Synthesizer`
...
import { clipId, getClip, googleSynthesizer, VOICES, type Synthesizer } from './index'
...
describe('googleSynthesizer', () => {
  it('requests the pinned voice and decodes the base64 audio', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValue(Response.json({ audioContent: Buffer.from([7, 8]).toString('base64') }))
    const out = await googleSynthesizer({ apiKey: 'k', fetchImpl }).synthesize('złośliwy', 'pl')
    expect([...out.bytes]).toEqual([7, 8])
    expect(out.voice).toBe(VOICES.pl)
    const body = JSON.parse(String(fetchImpl.mock.calls[0][1]!.body))
    expect(body.voice).toEqual({ languageCode: 'pl-PL', name: VOICES.pl })
    expect(body.input).toEqual({ text: 'złośliwy' })
  })

  it('throws on an API failure', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(new Response('nope', { status: 403 }))
    await expect(googleSynthesizer({ apiKey: 'k', fetchImpl }).synthesize('x', 'pl')).rejects.toThrow(
      /tts failed/,
    )
  })
})
...
export function googleSynthesizer(opts: { apiKey: string; fetchImpl?: typeof fetch }): Synthesizer {
  const doFetch = opts.fetchImpl ?? fetch
  return {
    async synthesize(text, lang) {
      ...
      return { bytes: new Uint8Array(Buffer.from(audioContent, 'base64')), mime: 'audio/mpeg', voice: VOICES[lang] }
    },
  }
}
...
export function getSynthesizer(): Synthesizer {
  ...
  return googleSynthesizer({ apiKey })
}
```
...
Makes the app reachable from the phone and leaves behind the one operational command that matters.
...
```bash
npm install
cp .env.example .env.local   # fill in APP_PASSWORD, SESSION_SECRET, the three API keys
...
The microphone, the service worker and home-screen install all require a secure
context, so a plain `http://192.168.x.x` address will not work. Tailscale
provides a real certificate:
...
Open the printed `https://<machine>.<tailnet>.ts.net` URL on the phone, log in
with `APP_PASSWORD`, then Chrome menu → *Add to Home screen*.

## Backup

```bash
./scripts/backup.sh            # writes backups/fiszki-YYYY-MM-DD-HHMM.db
```
...
| `FISZKI_DB` | database path, default `data/fiszki.db` |
| `FISZKI_MODEL` | Claude model, default `claude-opus-5` |
| `ANTHROPIC_API_KEY` | card generation |
| `GROQ_API_KEY` | Whisper transcription |
| `GOOGLE_TTS_API_KEY` | speech synthesis |
...
- [ ] **Step 5: Walk the app end to end on the phone**

With `npm start` and `tailscale serve` running, on the Android phone:
...
9. `./scripts/backup.sh` — confirm a file appears and opens.

- [ ] **Step 6: Commit**

```bash
git add scripts/backup.sh README.md lib/db/backup.test.ts
git commit -m "feat: backup script, deployment docs, full-suite gate"
```
